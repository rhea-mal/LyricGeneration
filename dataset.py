...
##this file doesn't use pandas-> using pandas could potentially make it better
#right now we are training w the first row of column declarations Artist, Song, Lyrics, etc
...
class SongDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_data = self.MASK_CHAR.join([row[x] for x in range(len(row)) if x != 2])  # All columns except the third (lyrics)
        output_data = row[2]              # Lyrics is the third column

        # Concatenate input and output with masking
        x = input_data + self.MASK_CHAR + output_data + self.MASK_CHAR
        x = x[:self.block_size] + self.PAD_CHAR * (self.block_size - len(x))  # Pad to ensure fixed length


        # Generate y by shifting x by the length of input_data minus 1
        y = self.PAD_CHAR * (len(input_data) - 1) + x[len(input_data):self.block_size]
        y = y + self.PAD_CHAR * (self.block_size - len(y)) ## Pad y if its length is less than block_size

        # Convert characters to indices using the vocabulary
...
if __name__ == '__main__':
    ...
    argp.add_argument('dataset_type', help="Type of dataset to sample from."
            "Options: songdata, charcorruption.",
            choices=["songdata", "charcorruption"])
    args = argp.parse_args()

    if args.dataset_type == 'songdata':
        ...
        name_dataset = SongDataset(open('train_data.csv', encoding='utf-8').read(), 2000)
