...
pretrain_dataset = dataset.SongDataset(text, block_size)

# We don't suggest you change these hyperparameters, as they're known to work.
# use them for both the vanilla and the perceiver models
mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
    n_layer=4, n_head=8, n_embd=256) #change parameters?
...
if args.function == 'pretrain':
    ...
    x = torch.tensor([pretrain_dataset.stoi[c] for c in song_characteristics], dtype=torch.long)
    prediction = utils.sample(model, x, steps=32, sample=False)
    print(prediction)
    """
    ...
    with open(args.outputs_path, 'w', encoding='utf-8') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path, encoding='utf-8')):
            x = line.split(',')
            x = '⁇'.join([x[i] for i in range(len(x)) if i != 2])
